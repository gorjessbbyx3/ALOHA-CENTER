...
export async function generateAppointmentPDF(appointment: Appointment, patient: Patient, service: Service): Promise<string> {
  ...
  const key = `pdfs/appointments/appointment_${appointment.id}.pdf`;
  const s3Path = await uploadBuffer(pdfBuffer, key, 'application/pdf');

  // For compatibility with existing code, also save locally
  const tempDir = os.tmpdir();
  const fileName = `appointment_${appointment.id}_${Date.now()}.pdf`;
  const pdfPath = path.join(tempDir, fileName);

  fs.writeFileSync(pdfPath, pdfBuffer);
  // Store both paths (You might want to only store the S3 path in the database)
  await storage.updateAppointment(appointment.id, {
    pdfPath: s3Path
  });

  return s3Path; // Returning S3 path
}
